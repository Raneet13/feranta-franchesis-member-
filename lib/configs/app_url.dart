class AppUrl {
  static var baseUrl = 'https://feranta.in/backend/api';
  static var placeSerach =
      'https://maps.googleapis.com/maps/api/place/autocomplete/json?';
  static var imageUrl = 'https://feranta.in/backend/uploads/';
  static var ferantaAbout = 'https://www.feranta.in/about-us/';
  static var ferantaPrivacy = 'https://www.feranta.in/privacy-policy/';
  static var ferantaTerms = 'https://www.feranta.in/term-and-condition/';
  static var ferantaCustomer = 'https://www.feranta.in/contact/';

  static var login = '/member-login';
  static var driverResister = '/register-driver';
  static var getallMaster = '/get-all-master';
  static var getList = '/get-list';
  static var customerRegistation = '/add-customer';
  // static var sendLivelocation = '/send-live-location';
  // static var book_service_request = '/book-service';
  static var profile = '/get-profile-details';
  // static var checkBookingdriver = '/check-booking-status';
  // static var getDriverDetails = '/get-profile-details';
  // static var myTripAll = '/get-customer-booking-history';
  static var updateProfile = '/updateProfile';
  // static var allNotification = '/get-all-notification';
  // static var cancelBooking = '/cancel-booking';

  ///book-service

  // static var popularMoviesListEndPoint =  'https://www.episodate.com/api/most-popular?page=1' ;
}
